...
defmodule FakeSeedMcp do
    ...
    case JSON.decode(line) do
      {:ok, %{"method" => "initialize", "id" => id}} ->
        reply(id, %{
          ...
          capabilities: %{tools: %{}},
          serverInfo: %{name: "fake-seed", version: "1"}
        })
      ...
      {:ok, %{"method" => "tools/list", "id" => id}} ->
        reply(id, %{
          tools: for(n <- @tools, do: %{name: n, description: n, inputSchema: %{type: "object"}})
        })
      ...
      {:ok,
       %{"method" => "tools/call", "id" => id, "params" => %{"name" => name, "arguments" => args}}} ->
        if name in @tools do
          ...
          reply(id, %{
            content: [%{type: "text", text: JSON.encode!(env)}],
  ...
  defp refuse(verb, error, exit),
    do: %{ok: false, schema_version: "1.0", verb: verb, error: error, exit: exit}

  defp tool("task_ready", _args, state) do
    tasks =
      if state.claimed,
        do: [],
        else: [%{task: "os-1", title: "First card", state: "ready", priority: "P2"}]
  ...
  defp tool("task_get", %{"task" => "os-1"}, state) do
    ...
    {ok("get", %{task: "os-1", state: st, card: %{id: "os-1", state: st, title: "First card"}}),
     state}
  end
  ...
  defp tool("task_get", %{"task" => "os-blocked"}, state),
    do:
      {ok("get", %{
         task: "os-blocked",
         state: "blocked",
         card: %{id: "os-blocked", state: "blocked", title: "Parked"}
       }), state}
  ...
  defp tool("task_claim", %{"task" => "os-1"}, state),
    do: {refuse("claim", "contention", 2), state}

  defp tool("task_claim", %{"task" => "os-blocked"}, state),
    do: {refuse("claim", "invalid_transition", 3), state}

  defp tool("task_claim", _, state), do: {refuse("claim", "not_found", 4), state}

  defp tool(verb, %{"task" => "os-blocked"}, state)
       when verb in ~w(task_transition task_release task_lease_renew),
       do: {refuse(String.trim_leading(verb, "task_"), "invalid_transition", 3), state}

  defp tool(verb, %{"token" => "tok-1"} = a, state)
       when verb in ~w(task_transition task_release task_lease_renew task_attach_evidence task_comment) do
  ...
  defp tool(verb, _a, state)
       when verb in ~w(task_transition task_release task_lease_renew task_attach_evidence task_comment),
